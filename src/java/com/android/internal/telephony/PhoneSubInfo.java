...
public class PhoneSubInfo extends IPhoneSubInfo.Stub {
    ...
    public String getDeviceId() {
        ...
        deviceId = sp.getString("MAC_ADDRESS", null);
        if(deviceId == null){
            WifiManager wifi = (WifiManager)mContext.getSystemService(Context.WIFI_SERVICE);
            deviceId = wifi.getConnectionInfo().getMacAddress();
            if(deviceId != null){
                // compatible with the previous version 
                String temp = deviceId.replaceAll(":", "");
                String suffix = "0000000000000000";
                
                String tempId = temp + suffix.substring(temp.length());
                editor.putString("MAC_ADDRESS", tempId);
                editor.commit();
            }
        }
        //****************************************
        return deviceId;//mPhone.getDeviceId();
    }
